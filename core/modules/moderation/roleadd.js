...
const init = async function (message) {
  ...
  const tgg = await gear.getTarget(message);
        const Target = message.mentions.members.first() || Server.member(tgg);
    ...
    if (!modPass) {
        return message.reply(mm('CMD.moderationNeeded', {
            ...
            function fR(role, memb) {
                ...
                let a = Server.roles.find(rol=>rol.name.toLowerCase()==role.toLowerCase()||rol.name.toLowerCase().includes(role.toLowerCase()));
                ...
                const roleadd_confirm = On + mm('CMD.superRoleadCom', {
                    lngs: LANG,
               ...
               // message.reply(role)
                ...
                memb.roles.add(a).then(a => message.channel.send(roleadd_confirm)).then(e => {
