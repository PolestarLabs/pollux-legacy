...
var init = async function (message,userDB,DB) { 
  ...
  //delete require.cache[require.resolve("../../archetypes/discoin.js")]
...
if (typeof args[1] != "string" || args[1].length != 3)return message.reply("invalid curr"); 
  ...
  if(!(await eko.checkFunds(args[0],message.author)))return message.reply(mm("$.noFundsGeneric",{lngs:message.lang}));
  ...
  const P={lngs:message.lang,$:$,amt:amt}
  const v = {}
  ...
  gear.finant.send("exg",{embeds:[embed]})
